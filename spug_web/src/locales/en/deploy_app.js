...
const dict = {
  ...
  '由 Moon 来控制发布的主流程，你可以通过添加钩子脚本来执行额外的自定义操作。': 'Moon drives the main deployment workflow, and you can add hook scripts to run extra custom steps.',
  '自定义发布': 'Custom Deploy',
  '你可以完全自己定义发布的所有流程和操作，Moon 负责按顺序依次执行你记录的动作。': 'You define every step of the deployment yourself, and Moon runs the actions you recorded in order.',
  ...
  '在运行 Moon 的服务器(或容器)上执行，当前目录为仓库源代码目录，可以执行任意自定义命令。': 'Runs on the server (or container) hosting Moon, with the repository source directory as the working directory. Any custom commands can be used.',
  ...
  '在运行 Moon 的服务器(或容器)上执行，当前目录为检出后的源代码目录，可执行任意自定义命令。': 'Runs on the server (or container) hosting Moon, with the checked-out source directory as the working directory. Any custom commands can be used.',
  ...
  '部署路径': 'Deploy path',
  '应用最终在主机上的部署路径，为了数据安全请确保该目录不存在，Moon 将会自动创建并接管该目录，可使用全局变量，例如：/www/$SPUG_APP_KEY': 'The path the app is deployed to on the target hosts. For data safety make sure it does not exist yet — Moon will create and take over this directory. Global variables are supported, e.g. /www/$SPUG_APP_KEY',
  ...
  'Moon 将遵循先本地后目标主机的原则，按照顺序依次执行添加的动作，例如：本地动作1 -> 本地动作2 -> 目标主机动作1 -> 目标主机动作2 ...': 'Moon runs the actions in order, local actions first and host actions second, e.g. local action 1 -> local action 2 -> host action 1 -> host action 2 ...',
  ...
  '添加本地执行动作（在服务端本地执行）': 'Add local action (runs on the Moon server)',
  '目标主机动作{}': 'Host action {}',
  '数据来源': 'Data source',
