...
MESSAGES = {
    '外部推送服务及其MFA设置已移除': 'The external push service and its MFA settings have been removed.',
    '外部推送MFA已移除，请联系管理员确认安全策略后执行 set mfa disable，再使用本地账户登录。': 'External push MFA has been removed. An administrator must review the security policy and run set mfa disable before local-account sign-in.',
    '推送助手节点已移除，请删除或替换该节点后保存。': 'Push Assistant nodes have been removed. Delete or replace the node before saving.',
    '推送助手节点已移除，请删除或替换该节点后执行。': 'Push Assistant nodes have been removed. Delete or replace the node before running the pipeline.',
    '该推送通道已移除，请重新选择节点模块': 'This push channel has been removed. Select another node module.',
    '报警方式已下线或不受支持，请重新选择报警方式': 'The alert method is retired or unsupported. Select a supported method.',
    '不支持的登录方式，请使用普通账户登录': 'Unsupported sign-in method. Please use a local account.',
    '历史外部账户无法修改密码': 'Passwords cannot be changed for historical external accounts',
    '历史外部账户无法删除，请使用禁用功能': 'Historical external accounts cannot be deleted; disable them instead',
    '该设置已下线': 'This setting has been retired',
...
CHOICES = {
    ...
    '进程检测': 'Process check',
    '自定义脚本': 'Custom script',
    'Docker服务检测': 'Docker service check',
...
_CONSOLE_PATTERNS = [
    ...
    (re.compile(r'^检测到该主机的发布目录 (.*) 已存在，为了数据安全请自行备份后删除该目录，Moon 将会创建并接管该目录。$', re.S),
     'The deploy directory {0} already exists on this host, please back it up and remove it yourself for data safety, Moon will create and take over this directory.'),
